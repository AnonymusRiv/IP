...
int sum_even(int v[], int n){		//creamos esta funcion para averiguar la suma de los valores pares que tenga el vector v
int suma=0;
for(int i=0;i<n;i++){		//para ver todos los elementos del vector se ejecuta este bucle
	if(v[i]%2==0){		//para comprobar si el numero en el termino i del vector es par se divide entre 2 y si el resto es cero se ejecuta esta accion
		suma+=v[i];		//se suma el numero del vector en el termino i 
		}
}
